...
async def translate_chunk_async(text: str, model_id: str = "gemini-3-flash-preview"):
    ...
    # Temperature 0.2 prevents the repetition loops
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Langchain AIMessage.content can sometimes be a list rather than a string
...
async def translate_all_chunks(chunks, model_id="gemini-3-flash-preview"):
    """Processes chunks with conservative rate limits for free tier API."""
    # Strict limits for gemini-3 free tier (15 Requests Per Minute)
    # 2 chunks every 10 seconds = 12 RPM, safely under the limit
    batch_size = 2  
    all_translations = []
